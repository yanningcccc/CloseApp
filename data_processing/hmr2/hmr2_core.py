...
class Human4D_Predictor(object):
    ...
    def vis_results(self, image, results, viz=False):
        ...
        front_view = np.concatenate((image, front_view, side_view, top_view), axis=1)

        renderer.delete()
        if viz:
            # cv2.imwrite('test.jpg', front_view)
            vis_img('img', front_view)

        return front_view


    def inference(self, item, viz=False, save_results=False):
        ...
        if viz or save_results:
            ...
            results.update(focal_length=[scaled_focal_length.detach().cpu().numpy().astype(np.float32)])
            img = cv2.imread(item['img_path'][0])
            rendered = self.vis_results(img, results, viz=viz)
        ...
        intri[0][2] = img_size[0,0] / 2
        intri[1][2] = img_size[0,1] / 2
        camparams = {'intri':[intri], 'extri':[extri]}

        params = {}
        params.update(img_path=item['img_path'][0])
        params.update(pose=pose)
        params.update(betas=betas)
        params.update(trans=pred_cam_t_full)
        params.update(features=features)
        params.update(camparams=camparams)
        
        if save_results:
            params.update(pred_vertices=pred_vertices)
            params.update(rendered=rendered)
    ...
    def predict(self, img_path, boxes, viz=False, save_render=False):
        item = {}
        ...
        if save_render:
            rendered = self.vis_results(img_origin, results, viz=viz)
        else:
            rendered = None
        ...
        intri[0][0] = scaled_focal_length.detach().cpu().numpy()
        intri[1][1] = scaled_focal_length.detach().cpu().numpy()
        intri[0][2] = img_origin.shape[1] / 2
        intri[1][2] = img_origin.shape[0] / 2
    ...
    def save_results(self, params, rendered, camparams, verts, img_folder):
        name = os.path.basename(img_folder)
        out_folder = os.path.join('output/Human4D', name) 
