...
class HumanEval(nn.Module):
    def __init__(self, name, generator=None, dtype=torch.float32, **kwargs):
        ...
        self.neutral_smpl = SMPLModel(model_path='smpl/smpl/SMPL_NEUTRAL.pkl', data_type=dtype)
        self.male_smpl = SMPLModel(model_path='smpl/smpl/SMPL_MALE.pkl', data_type=dtype)
        self.female_smpl = SMPLModel(model_path='smpl/smpl/SMPL_FEMALE.pkl', data_type=dtype)

        self.dtype = dtype

        self.name = name
        self.J_regressor_H36 = np.load('smpl/J_regressor_h36m.npy').astype(np.float32)
        self.J_regressor_LSP = np.load('smpl/J_regressor_lsp.npy').astype(np.float32)
        self.J_regressor_halpe = np.load('smpl/J_regressor_halpe.npy').astype(np.float32)
        ...
        self.eval_handler_mapper = dict(
            ...
            MPI3DPW_OC=self.LSPEvalHandler,
    ...
    def report(self):
        ...
        print("PVE: %f, MPJPE: %f, PA-MPJPE: %f, Accel: %f, Inter: %f, J-PA-MPJPE: %f" %(vertex_error, error, error_pa, accel, interaction, joint_error_pa))
    ...
    def forward(self, pred_params, gt_params, eval_interaction=False):

        for seq in tqdm(pred_params['pose'].keys(), total=len(pred_params['pose'])):
            pred_pose = np.array(pred_params['pose'][seq])
            pred_shape = np.array(pred_params['shape'][seq])[...,:10]
            ...
            for idx in range(num_agent):
                ...
                # p_valid = abs(pred_pose[:,idx]).sum(axis=1) != 0

                p_pose = torch.from_numpy(pred_pose[:,idx]).contiguous()#[p_valid]
                p_shape = torch.from_numpy(pred_shape[:,idx]).contiguous()#[p_valid]
                p_trans = torch.from_numpy(pred_trans[:,idx]).contiguous()#[p_valid]

                g_pose = torch.from_numpy(gt_pose[:,idx]).contiguous()#[p_valid]
                g_shape = torch.from_numpy(gt_shape[:,idx]).contiguous()#[p_valid]
                g_trans = torch.from_numpy(gt_trans[:,idx]).contiguous()#[p_valid]

                v = valid[:,idx]#[p_valid]

                pred_mesh, pred_joint = self.neutral_smpl(p_shape, p_pose, p_trans, lsp=True)
                ...
                # for i, loss in enumerate(error):
                #     if loss > 200:
                #         write_obj(gt_mesh[i], self.neutral_smpl.faces, 'output/gt000.obj')
                #         write_obj(pred_mesh[i], self.neutral_smpl.faces, 'output/pred000.obj')

                gt_meshes.append(gt_mesh)
                pred_meshes.append(pred_mesh)
                inter_valid.append(v)
                gt_joints.append(gt_joint)
                pred_joints.append(pred_joint)

            if eval_interaction:
                valids = np.array(inter_valid).sum(axis=0)
                gt_joints = np.array(gt_joints).transpose((1,0,2,3)).reshape(len(valids), -1, 3) * 1000
                pred_joints = np.array(pred_joints).transpose((1,0,2,3)).reshape(len(valids), -1, 3) * 1000
                valids[valids<2] = 0.
                valids[valids==2] = 1.
                _, joint_error, joint_error_pa, _, _, _ = self.compute_errors(gt_joints, pred_joints, valids, confs=None, format='lsp')
                self.joint_error_pa += joint_error_pa

                inter_loss = self.calcu_interaction(gt_meshes, pred_meshes, inter_valid)
                self.inter_loss += inter_loss

    def calcu_interaction_origin(self, gt_meshes, pred_meshes, inter_valid):
        valid = np.array(inter_valid).sum(axis=0)

        gt_joints_c1 = self.J_regressor_LSP @ gt_meshes[0] * 1000
        gt_joints_c2 = self.J_regressor_LSP @ gt_meshes[1] * 1000

        pred_joints_c1 = self.J_regressor_LSP @ pred_meshes[0] * 1000
        pred_joints_c2 = self.J_regressor_LSP @ pred_meshes[1] * 1000

        gt_interaction = gt_joints_c1[valid==2] - gt_joints_c2[valid==2]
        pred_interaction = pred_joints_c1[valid==2] - pred_joints_c2[valid==2]

        inter_loss = np.mean(np.sqrt(np.sum((gt_interaction - pred_interaction) ** 2, axis=-1)), axis=-1).tolist()
    ...
    def forward_romh(self, pred_params, gt_params, eval_interaction=False):
        self.halpe2lsp = [16,14,12,11,13,15,10,8,6,5,7,9,18,17]

        for i, seq in enumerate(tqdm(pred_params, total=len(pred_params))):

            pred_joints = pred_params[i][:,self.halpe2lsp].detach().cpu().numpy()
            gt_joint = gt_params[i][:,self.halpe2lsp].detach().cpu().numpy()

            pred_joints = pred_joints * 1000
            gt_joint = gt_joint * 1000

            valids = np.ones((len(gt_joint)))

            abs_error, error, error_pa, abs_pck, pck, accel = self.compute_errors(gt_joint, pred_joints, valids, format='lsp')



            self.LSPEvalHandler()

            vertex_error, error, error_pa, abs_pck, pck, accel = self.evaluate(pred_joints, gt_joint, v)
            self.vertex_error += vertex_error
            self.error += error
            self.error_pa += error_pa
            self.abs_pck += abs_pck
