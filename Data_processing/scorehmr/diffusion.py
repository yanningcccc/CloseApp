...
from scorehmr.utils.guidance_losses import *
import pickle
...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(self, cfg: CfgNode, model: nn.Module, **kwargs) -> None:
        ...
        if self.use_guidance:
            ...
            self.transl_temperal = False
            self.contact_guidance = True
        ...
        if self.objective == "pred_noise":
            ...
            if self.use_guidance and not inverse:
                ...
                if timestep <= self.sample_start:
                    ...
                    scaled_gradient, mask = self.cond_fn(x, t_, pred_noise, mv_batch, frame_num, step, time)
    ...
    def cond_fn(
        self, x: torch.Tensor, t_: torch.Tensor, pred_noise: torch.Tensor, mv_batch: Dict, frame_num: int, step: int, time: int,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        """
        batch = mv_batch[0]
        real_batch_size = batch["joints_2d"].size(0)
        num_samples = x.size(0) // real_batch_size
        agent_num = x.size(0) // frame_num
        ...
        with torch.enable_grad():
            ...
            if self.keypoint_guidance:
                ...
                if self.optimize_shape:
                    pred_betas = self.body_shape.unsqueeze(1).repeat(1, real_batch_size//agent_num, 1).reshape(real_batch_size * num_samples, -1)
                else:
                    pred_betas = (
                ...
                # Contact loss
                if self.contact_guidance and batch['contact'] is not None:
                    pred_verts = smpl_output.vertices
                    # loss_temp = self.cfg.GUIDANCE.W_CONTACT * contact_loss(pred_verts, contact_label, rid_to_vid, frame_num)
                    loss_contact = self.cfg.GUIDANCE.W_CONTACT * contact_loss(pred_verts, self.camera_translation, batch, frame_num, agent_num)
                else:
                    loss_contact = 0.
                ...
                loss_transl = 0.
                if self.transl_temperal:
                    loss_transl = transl_smoothness_loss(self.camera_translation, frame_num, agent_num) 
                    loss_transl = self.cfg.GUIDANCE.W_TRANSL * loss_transl

                loss = self.cfg.GUIDANCE.W_KP2D * loss_kp

                ## Update camera translation.
                loss_transl = loss_transl + loss_contact + loss.mean()
                self.camera_translation_optimizer.zero_grad()
                loss_transl.backward(retain_graph=True)
                # print(self.camera_translation[0])
                self.camera_translation_optimizer.step()

            if self.optimize_shape:
                loss_shape = self.cfg.GUIDANCE.W_SHAPE * shape_loss(pred_betas, batch['pred_betas'])
                print(loss_shape)
                ## Update body shape.
                loss_shape = loss_shape + loss_transl + loss_contact + loss.mean()
                self.body_shape_optimizer.zero_grad()
                loss_shape.backward(retain_graph=True)
                # print(self.camera_translation[0])
                self.body_shape_optimizer.step()
            ...
            if self.temporal_guidance:
                loss_temp = self.cfg.GUIDANCE.W_SMOOTH * smoothness_loss(pred_pose_6d, pred_keypoints_3d, frame_num, agent_num)
                if self.keypoint_guidance:
                    # print(loss_temp)
                    gradient = torch.autograd.grad(loss.mean() + loss_temp + loss_contact, x_t)[0]
    ...
    @torch.no_grad()
    def sample(self, mv_batch: Dict, cond_feats: torch.Tensor, frame_num: int, batch_size: int):
        """
        ...
        return sample_fn(mv_batch, cond_feats, frame_num, shape)
    ...
    def ddim_with_guidance(
        ...
        if self.optimize_shape:
            self.body_shape = batch["pred_betas"].reshape(-1, frame_num, 10)[:,0].detach()
            self.body_shape.requires_grad_(True)
            self.body_shape_optimizer = torch.optim.Adam([self.body_shape], lr=1e-2)
        else:
            self.body_shape = None

        if self.contact_guidance and batch['contact'] is not None:
            rid_to_vid = batch['contact']['rid_to_vid']
            contact_label = batch['contact']['label']
            assert len(contact_label) == frame_num
            ...
            for i, labels in enumerate(contact_label):
                ...
                for label in labels:
                    frame_idx.append(i)
                    # print(len(rid_to_vid[label[0]]))
                    # print(len(rid_to_vid[label[1]]))
                    verts1_idx.append(rid_to_vid[label[0]][:100] + self.smpl.v_template.shape[0] * num)
                    verts2_idx.append(rid_to_vid[label[1]][:100] + self.smpl.v_template.shape[0] * num)
                    num += 1
                    pair.append([rid_to_vid[label[0]].tolist(), rid_to_vid[label[1]].tolist()])
                contact_verts.append(pair)
            batch['contact_verts'] = contact_verts
            batch['contact_idx'] = frame_idx
            batch['verts1_idx'] = verts1_idx
            batch['verts2_idx'] = verts2_idx
        ...
        print('Optimizing')
        for step in tqdm(range(self.optim_iters), total=self.optim_iters):
        ...
        if self.body_shape is not None:
            body_shape = self.body_shape.detach().unsqueeze(1).repeat(1, frame_num, 1).reshape(-1, 10)
        else:
            body_shape = batch['pred_betas']
        output = (
                    self.early_stop_obj.get_preds(x_start, self.camera_translation)
                    if self.early_stop
                    else {'x_0': x_start, 'camera_translation': self.camera_translation, 'betas':body_shape}
        )
        return output
