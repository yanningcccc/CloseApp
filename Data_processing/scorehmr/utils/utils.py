...
from utils.module_utils import filter_butter
...
def prepare_smpl_params(
    ...
    pred_pose = pred_pose.reshape(agent_num, frame_num, -1)

    trans = trans.detach().cpu().numpy()
    trans = trans.reshape(agent_num, frame_num, -1)

    for i, pose in enumerate(pred_pose):
        pred_pose[i] = filter_butter(pose, N=3, Wn=0.7)

    for i, t in enumerate(trans):
        trans[i] = filter_butter(t, N=3, Wn=0.7)


    pred_pose = torch.from_numpy(pred_pose.reshape(-1, 144)).cuda()
...
class StandarizeImageFeatures:
    def __init__(
        ...
        feat_stats = np.load(f"pretrained/ScoreHMR_data/stats/{backbone}_feat_stats.npz")
        if backbone == "prohmr":
